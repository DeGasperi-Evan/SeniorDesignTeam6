...
if __name__ == '__main__':
	...
	try:
		...
		sleep(0.01)
		out = os.open(outPath, os.O_WRONLY) #open pipe
		infile = os.open(inPath, os.O_RDONLY) #open pipe
	except:
		try:
			os.unlink(outPath) #close pipe
			os.unlink(inPath) #close pipe
			os.unlink(lidar) #close lidar pipe <3 
		except:
	...
	anchors = [DeviceCoordinates(0x7604, 1, Coordinates(5734,11275,1370)),
			   ...
			   DeviceCoordinates(0x6A10, 1, Coordinates(2145,2947,1250)),
	...
	r.setup()
	pos = Coordinates() #I found the Pos! (noPos == False)
	mag = Magnetic()
	head = EulerAngles()
	...
	#-----------Create CSV File -------NIC_ 11/14/2022
	##now = datetime.now()
	#dt_string = now.strftime("%m%d%Y-%H_%M_%S")
	#filename = "Chorette_" + dt_string #str(date.today()) + 
	#print(filename)
...
# creating a csv writer object 
	...
	#-----------LIDAR: EXPERIMENTAL _ Initializaiton-----------
	...
	while True:
		...
		try:
			...
			#-------LIDAR_ Experimental------- <---THERE SHOULD NOT BE ANY MORE LIDAR BELOW THIS LINE, DELEATED A ***LOT*** OF PSEUDOCODE, IF THERE IS ANY IM SORRY 
			read = datetime.now()
			...
			os.write(out, bytes(f"{pos.x/1000},{pos.y/1000},{head.heading}\n","utf-8")) #line pipes data over to C, have fun parsing lidar sting <3***CORRECTION, THEY ARE NOT RN, BUT THEY ***COULD*** BE USED FOR IT, IF "async" is used, (if it does what Nic Chorette thinks it can do, run in background forever [if u want it to], and share a variable, aka memory, all in 1 python file, w/o the need for more pipes, but, reguardless, yeah, work same idea of run it in parallel with this code is what i THINK asynch does, but... if I got it truly working, there is no need for that )

			print(pos)
			[motorduty, servoduty] = [float(x) for x in os.read(infile, 512).decode("utf-8").rstrip("\x00").split(",")] #read C-> python Pipe to set servo Stuffs
			...
			#--WARNNING: THIS MAY RESULT IN THE CAR GOING SLOWER!!! [IT HAS TO WRITE TO A FILE AFTERALL!!!) 
			#addSensors = [pos.x, pos.y, servoduty, motorduty, head.heading]
			#csvwriter.writerow(addSensors)

			
			
		except: #Run when u stop the program 
			try:
				print("Stopping Program (Python Side...) ")
				servopwm.change_duty_cycle(72)
				motorpwm.change_duty_cycle(72)
				os.close(out)
				os.close(infile)
				os.close(lidar) #my belovid <3
				os.unlink(outPath)
				os.unlink(inPath)
				os.unlink(lidar) #close lidar pipe <3 
				...
				os.system("pkill python3") #it works sometimes.... not good enough 
				#Kill any remaining python3 programs (aka, my personal lidar one, that does not close right for some reaosn )
				#os.system("pkill python3") #dangerous command, kinda, but not rly
			except:
				print("Files already deleted\n")
				#os.system("pkill python3") #same thing as in the try statemtent 
			exit()
